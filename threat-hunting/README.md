...
| H2 | [LSASS Reconnaissance](hunt2-lsass-recon.md) | LSASS was accessed with masks or source processes not covered by the existing T1003.001 rule | Sysmon ID 10 | **Gap analysis complete** — svchost+0x1410 identified as benign false positive pattern | T1003.001 rule updated with svchost+0x1410 and wmiprvse conditional exclusion |
...
| H4 | [C2 Beaconing](hunt4-c2-beaconing.md) | A host may be beaconing to a C2 server via HTTP traffic with anomalous user agents or connection patterns | Suricata HTTP flows, Sysmon ID 3 | **Confirmed** — anomalous HTTP user agents and PowerShell outbound connections to 192.168.75.11:8080 identified; burst pattern consistent with simulated C2 (T1071.001) | Gap resolved — added `T1071.001-suspicious-script-egress` rule for process-level visibility |
---
